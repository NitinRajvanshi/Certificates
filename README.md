# 📜 Certifications

![Cisco](https://img.shields.io/badge/Cisco-Networking_Academy-blue)

![IIT_BHU](https://img.shields.io/badge/IIT_BHU-Short_Course-orange)

![ELC](https://img.shields.io/badge/ELC-Summer_Internship-green)

This repository contains my verified technical certifications, internship completion certificates, and professional learning achievements.

| Certification | Organization | Year |
|---------------|--------------|------|
| Cisco Cybersecurity Essentials | Cisco Networking Academy | 2026 |
| Summer Internship Certificate | Experiential Learning Centre (ELC), TIET | 2025 |
| Introduction to Quantum Communication and Machine Learning | IIT (BHU) Varanasi | 2024 |

## Repository Structure

```text
Cisco/
ELC_TIET/
IIT_BHU/
```

Each folder contains the corresponding certificate and supporting information.
